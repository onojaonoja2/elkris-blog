...
@section('title', 'Create Post - Elkris Bio Health')
...
<form method="POST" action="{{ route('admin.posts.store') }}" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-8" id="post-form">
    ...
    <div class="lg:col-span-2 space-y-6">
        <div class="bg-white rounded-xl border border-surface-variant shadow-sm p-6">
            <input type="text" name="title" id="title" value="{{ old('title') }}" placeholder="Post title..." class="w-full text-[24px] md:text-[32px] font-headline-md font-semibold text-primary border-none outline-none focus:outline-none placeholder:text-outline-variant bg-transparent" required/>
        ...
        <div class="bg-white rounded-xl border border-surface-variant shadow-sm">
            ...
            <div class="p-6">
                ...
                <input type="hidden" name="body" id="body-content" value="{{ old('body') }}">
        ...
        <div class="bg-white rounded-xl border border-surface-variant shadow-sm p-6">
            ...
            <textarea name="excerpt" rows="3" class="w-full border border-outline-variant rounded-lg px-4 py-3 text-ui-label text-on-surface focus:outline-none focus:ring-2 focus:ring-secondary/20 focus:border-secondary bg-surface-container-lowest" placeholder="Brief summary for cards and SEO...">{{ old('excerpt') }}</textarea>
    ...
    <div class="space-y-6">
        <div class="bg-white rounded-xl border border-surface-variant shadow-sm p-6 space-y-4 sticky top-24">
            ...
            <div>
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    <label class="inline-flex items-center gap-2 px-3 py-2 rounded-lg border border-outline-variant cursor-pointer hover:bg-surface-container-high transition-colors has-checked:bg-secondary-container has-checked:border-secondary has-checked:text-on-secondary-container">
                        <input type="checkbox" name="tags[]" value="{{ $tag->id }}" class="hidden" @checked(in_array($tag->id, old('tags', [])))>
            ...
            <div class="border-t border-outline-variant pt-4">
                ...
                <div class="space-y-3">
                    <input type="text" name="seo_title" value="{{ old('seo_title') }}" class="w-full border border-outline-variant rounded-lg px-4 py-3 text-ui-label text-on-surface focus:outline-none focus:ring-2 focus:ring-secondary/20 focus:border-secondary bg-surface-container-lowest" placeholder="SEO Title (leave blank to use post title)">
