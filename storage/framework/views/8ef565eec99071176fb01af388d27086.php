<?php $__env->startSection('title', ($post->seo_title ?? $post->title) . ' - Elkris Bio Health'); ?>
<?php $__env->startSection('meta_description', $post->seo_description ?? $post->excerpt ?? ''); ?>

<?php $__env->startPush('seo'); ?>
<meta property="og:title" content="<?php echo e($post->seo_title ?? $post->title); ?> - Blood Sugar Friendly Foods">
<meta property="og:description" content="<?php echo e($post->seo_description ?? $post->excerpt ?? ''); ?>">
<meta property="og:type" content="article">
<meta property="og:url" content="<?php echo e(route('blog.show', $post)); ?>">
...
<script type="application/ld+json">
{
  ...
  "description": "<?php echo e($post->seo_description ?? $post->excerpt ?? ''); ?>",
  "keywords": "blood sugar friendly foods, diabetes nutrition, <?php echo e($post->category?->name ?? ''); ?>, healthy alternatives, low glycemic foods",
  "author": {
    "@type": "Person",
    "name": "<?php echo e($post->author?->name ?? 'Elkris Bio Health'); ?>"
  },
  ...
  <?php if($post->featured_image): ?>
  ,"image": "<?php echo e(Storage::url($post->featured_image)); ?>"
  <?php endif; ?>
}
</script>
<?php $__env->stopPush(); ?>
...
<div class="flex flex-col gap-6">
    ...
    <?php endif; ?>
    <h1 class="font-display-lg-mobile md:font-display-lg text-[32px] md:text-[48px] leading-tight font-bold text-primary"><?php echo e($post->title); ?></h1>
    <div class="flex items-center gap-4 text-ui-label text-on-surface-variant">
            <div class="flex items-center gap-3">
                <?php if($post->author?->avatar): ?>
                <img src="<?php echo e($post->author->avatar_url); ?>" alt="<?php echo e($post->author->name); ?>" class="w-10 h-10 rounded-full object-cover">
                <?php else: ?>
                <div class="w-10 h-10 rounded-full bg-secondary-container flex items-center justify-center text-on-secondary-container font-bold">
                    <?php echo e(strtoupper(substr($post->author?->name ?? 'E', 0, 1))); ?>

                </div>
                <?php endif; ?>
            <div class="flex flex-col">
                <span class="font-semibold text-primary"><?php echo e($post->author?->name ?? 'Elkris Bio Health'); ?></span>
                <span class="text-caption text-outline"><?php echo e($post->published_at?->format('F j, Y')); ?> &bull; <?php echo e($post->reading_time); ?></span>
            </div>
        </div>
    </div>
</div>
...
<p class="text-caption text-outline mt-2 text-center"><?php echo e($post->featured_image_caption); ?></p>
<?php endif; ?>
</div>
<?php endif; ?>
...
<?php endif; ?>
<?php if($post->featured_image && $post->image_position === 'end'): ?>
<div class="mt-8 rounded-xl overflow-hidden shadow-xl">
    <img alt="<?php echo e($post->featured_image_caption ?? $post->title); ?>" class="w-full" src="<?php echo e(Storage::url($post->featured_image)); ?>"/>
<?php if($post->featured_image_caption): ?>
<p class="text-caption text-outline mt-2 text-center"><?php echo e($post->featured_image_caption); ?></p>
<?php endif; ?>
</div>
...
<div class="flex flex-wrap gap-2 mb-8">
    ...
    <a href="<?php echo e(route('blog.tag', $tag)); ?>" class="inline-flex items-center px-3 py-1 rounded-full border border-outline-variant text-on-surface-variant text-caption hover:bg-surface-container-high hover:border-secondary transition-all no-underline">#<?php echo e($tag->name); ?></a>
    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
</div>

<div class="flex items-center gap-4 p-6 bg-surface-container-low rounded-xl">
    <?php if($post->author?->avatar): ?>
    <img src="<?php echo e($post->author->avatar_url); ?>" alt="<?php echo e($post->author->name); ?>" class="w-14 h-14 rounded-full object-cover">
    <?php else: ?>
    ...
    <?php endif; ?>
    <div>
        <span class="font-ui-label font-bold text-primary">Written by <?php echo e($post->author?->name ?? 'Elkris Bio Health'); ?></span>
        <p class="text-caption text-outline mt-1">Published <?php echo e($post->published_at?->diffForHumans()); ?></p>
    </div>
</div>
</div>
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6">
    ...
    <a href="<?php echo e(route('blog.show', $related)); ?>" class="bg-white rounded-xl overflow-hidden shadow-sm border border-surface-variant group cursor-pointer hover:shadow-md transition-shadow no-underline">
        <div class="aspect-video bg-surface-container-high relative overflow-hidden">
            <?php if($related->featured_image): ?>
            <img alt="<?php echo e($related->title); ?>" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" src="<?php echo e(Storage::url($related->featured_image)); ?>"/>
            <?php else: ?>
        ...
        <div class="p-6">
            <span class="text-secondary font-bold text-caption tracking-widest uppercase"><?php echo e($related->category?->name ?? 'General'); ?></span>
            <h4 class="font-headline-sm text-[24px] font-semibold text-primary group-hover:text-secondary transition-colors mt-2"><?php echo e($related->title); ?></h4>
            <p class="text-ui-label text-on-surface-variant line-clamp-2 mt-2"><?php echo e($related->excerpt); ?></p>
            <div class="flex items-center gap-2 mt-4 text-caption text-outline">
                <span><?php echo e($related->published_at?->diffForHumans()); ?></span>
                <span>&bull;</span>
                <span><?php echo e($related->reading_time); ?></span>
            </div>
        </div>
    </a>
    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
...
<?php echo $__env->make('layouts.public', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Onoja\Documents\Elkris\Projects\elkris-blog\resources\views/blog/show.blade.php ENDPATH**/ ?>